...
namespace adb::protocol
{
     ...
     */
    std::string host_message(boost::asio::ip::tcp::socket& socket);
     ...
     */
    std::string host_data(boost::asio::ip::tcp::socket& socket);
     ...
     * @throw std::runtime_error Thrown on socket failure.
     */
    void sync_response(boost::asio::ip::tcp::socket& socket, std::string& id, uint32_t& length);
     ...
     */
    void send_host_request(boost::asio::ip::tcp::socket& socket, const std::string_view request);
     ...
     * @throw std::runtime_error Thrown on socket failure.
     */
    void send_sync_request(boost::asio::ip::tcp::socket& socket, const std::string_view id, const uint32_t length,
                           const char* body);
} // namespace adb::protocol
